...
passing tests — updated below to reflect what was actually built, not
the original plan.

| # | Criterion | Verified by | Phase |
|---|---|---|---|
| 1 | A learner can study one principle through multiple sub-lessons | **Done, content-complete for all three curated units** (`meet-the-pieces`, `check-and-checkmate`, `basic-tactics`) — `e2e/learning-path.spec.ts`'s "shows principle groupings with a mastery badge" test exercises the unit-agnostic grouping/gating code path against `meet-the-pieces`; `check-and-checkmate`/`basic-tactics` aren't separately E2E-covered but use the identical data-driven mechanism (`lib/principles.ts`, `LearningPath.tsx`), no unit-specific code. Sub-lesson sequencing within a principle reuses ADR-0007's existing per-lesson `prerequisites` check, unchanged | A |
...
| 3 | The next principle unlocks only after defined proficiency | **Done, content-complete for all three curated units** — `e2e/learning-path.spec.ts`'s "completing a principle's lessons sloppily doesn't unlock the next principle" and "strong performance ... unlocks the next one" tests drive this against real `UserConceptMastery` data for `meet-the-pieces`; the server-side gate (`app/learn/[lessonId]/page.tsx`) is the same unit-agnostic check for every unit with a principle file. Proficiency signal today is exercise-attempt accuracy only (`lib/masteryModel.ts`) — puzzle accuracy and a per-principle mastery-challenge result aren't part of it yet, since neither exists | A |
