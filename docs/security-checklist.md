...
| Rate limiting | **Implemented, as a stopgap.** `apps/web/lib/rate-limit.ts` — an in-memory sliding-window limiter applied to both `loginAction` (per-IP and per-email) and `signupAction` (per-IP). Per-process state only; doesn't survive a restart or share state across instances. Also collapses many real users into one bucket behind shared NAT or any deploy without a reverse proxy setting `x-forwarded-for` (falls back to a literal `"unknown"` key) — deliberately generous limits, not a fix. Postgres now exists (ADR-0005) as a plausible place to back a real shared-store version, but it isn't built. See `docs/known-risks.md`. |
