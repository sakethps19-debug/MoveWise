...
export default async function PlayPage() {
  ...
  // A real game review's recommendations (computed client-side, then
  // resolved server-side, ADR-0008 Phase B) can point at any lesson id,
  // not a fixed known-in-advance set — needs filesystem access
  // (lib/lessons.ts), so resolved server-side like every other lesson
  // lookup in this app.
  ...
  return (
    <div className="mw-app-shell">
      ...
      <main style={{ maxWidth: 1100, margin: "0 auto", padding: "var(--mw-space-6) var(--mw-space-4)" }}>
        ...
        <PlayRunner lessonTitleById={lessonTitleById} signedIn={user !== null} />
      </main>
    </div>
  );
}
